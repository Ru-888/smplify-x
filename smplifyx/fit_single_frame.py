...
def fit_single_frame(img,
    ...
    gt_joints = keypoint_data[:, :, :2] # 取X，Y 2D坐标
    # if use_joints_conf:
    #     joints_conf = keypoint_data[:, :, 2].reshape(1, -1)
    gt_joints3d = keypoint3d_data[:, :, :3]  # 取X，Y， Z  3D 坐标
    ...
    print("init_joints_idxs", init_joints_idxs)
    edge_indices = kwargs.get('body_tri_idxs')
    print("edge_indices", edge_indices)
    #
    # print("camera.focal_length_x: ", camera.focal_length_x, "camera.focal_length_y: ", camera.focal_length_y)
    # print("camera.rotation: ", camera.rotation)
    # print("camera.center: ", camera.center)
    print("camera.translation: ", camera.translation.detach())
    # gt_joints2d = fitting.joints3d_joints2d(gt_joints, camera)
    # guess_init函数用于初始化相机的平移向量
    ...
    camera_loss.trans_estimation[:] = camera.translation.detach()
    print("camera_loss.trans_estimation[:]: ", init_t)
    loss = fitting.create_loss(loss_type=loss_type,
                               gt_joints=gt_joints3d,
    ...
    with fitting.FittingMonitor(
        ...
        camera_opt_params = [body_model.global_orient]
        ...
        fit_camera = monitor.create_fitting_closure(
            camera_optimizer, body_model, camera, gt_joints3d,
            camera_loss, create_graph=camera_create_graph,
            use_vposer=use_vposer, vposer=vposer,
            pose_embedding=pose_embedding,
            return_full_pose=False, return_verts=False)

        # Step 1: Optimize over the torso joints the camera translation 优化躯干关节的相机平移
        # Initialize the computational graph by feeding the initial translation
        # of the camera and the initial pose of the body model. 通过输入相机的初始平移和身体模型的初始姿态来初始化计算图。
        ...
        # store here the final error for 0000, 1751.7611]], both orientations,
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            print("or_idx, orient: ", or_idx, orient)
            opt_start = time.time()
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc='Stage')):
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    camera=camera, gt_joints=gt_joints3d,
