...
'''
命名元组的好处是代码更加清晰，因为你不需要记住每个位置的字段名，直接通过字段名访问即可
'''
...
def read_keypoints(keypoint_fn, use_hands=True, use_face=True,
    ...
    dtype = np.float32)
        ...
        body_keypoints = body_keypoints.reshape([-1, 3])  # -1表示自动计算该维度的大小，以便保持数组中元素的总数不变。3表示每个关键点将有三个坐标值
...
class OpenPose(Dataset):
    ...
    def read_item(self, img_path):
        ...
        output_dict = {'fn': img_fn,
                       ...
                       'keypoints3d': keypoints3d, 'img': img,}
